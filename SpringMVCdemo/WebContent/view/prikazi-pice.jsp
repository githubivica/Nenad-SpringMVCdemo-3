...
<body>
	...
	<form:form modelAttribute="piceZaNarucivanje" action = "dodajPiceNaRacun" method="post">
		
		IME: ${piceZaNarucivanje.imeArtikla}<br>
		CENA: ${piceZaNarucivanje.cena}<br><br>
		<form:hidden path="idArtikla"/>
		<form:hidden path="imeArtikla"/>
		<form:hidden path="cena"/>
		<form:hidden path="stanje"/>
		<form:hidden path="popust"/>
		<form:hidden path="zapremina"/>
		KOLICINA: 
		<select name="kolicina">
			<c:forEach var = "i" begin="1" end="${piceZaNarucivanje.stanje}">
